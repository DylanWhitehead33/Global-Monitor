...
(function () {
  ...
  /* ---------- studio voice (ElevenLabs recording generated by the Action) ---------- */
  let studioAudio = null;
  let analyserRAF = null;
  async function tryStudioDebrief() {
    const deb = await fetchJSON("data/debrief.json");
    if (!deb?.text || !deb.updated) return null;
    if (Date.now() - Date.parse(deb.updated) > 26 * 3600e3) return null;   // stale — use live browser voice
    return deb;
  }
  function presentStudio(deb, done) {
    speaking = true;
    cancelled = false;
    textEl.textContent = "";
    const audio = new Audio(`data/debrief.mp3?cb=${encodeURIComponent(deb.updated)}`);
    studioAudio = audio;
    const text = deb.text;
    const times = deb.times ?? [];

    // orb + waveform driven by the actual audio signal
    let analyser = null;
    try {
    ...
    const tick = () => {
      ...
      // transcript sync from character timestamps
      if (times.length) {
        while (charPtr < times.length && times[charPtr] <= t) charPtr++;
        textEl.textContent = text.slice(0, charPtr);
      }
      // orb follows the voice's real amplitude
      if (analyser) {
        analyser.getByteFrequencyData(freq);
        let sum = 0;
        for (let i = 1; i < 24; i++) sum += freq[i];   // voice band
        const level = sum / (23 * 255);
        orb.style.transform = `scale(${(1 + level * 0.22).toFixed(3)})`;
        core.style.boxShadow = `0 0 ${40 + level * 60}px var(--accent), 0 0 ${90 + level * 120}px rgba(0,229,255,.6)`;
        for (let i = 0; i < bars.length; i++) {
          bars[i].style.height = (4 + (freq[(i * 2) % freq.length] / 255) * 28).toFixed(0) + "px";
        }
      }
    ...
    audio.play().then(() => {
      if (!analyser) waveOn();   // fallback animation if Web Audio was blocked
      tick();
    }).catch(() => {
      // playback refused (e.g. codec/policy) — fall back to the browser voice
      finish();
      present(text, done);
    });
  }
  ...
  function dismiss() {
    ...
    try { studioAudio?.pause(); } catch (_) { /* ignore */ }
    cancelAnimationFrame(analyserRAF);
    waveOff();
  ...
  briefBtn.addEventListener("click", async () => {
    ...
    const studio = await tryStudioDebrief();
    if (studio) {
      const rec = new Date(studio.updated);
      subEl.textContent = `SECURE VOICE CHANNEL · DELIVERING DEBRIEF · RECORDED ${rec.toISOString().slice(11, 16)} UTC`;
      presentStudio(studio, onDone);
    } else {
      subEl.textContent = "SECURE VOICE CHANNEL · DELIVERING DEBRIEF";
      const script = await buildDebrief();
      present(script, onDone);
    }
  });
